...
programme :
		PROG corps { $$ = $2 ; insererRegion( 0,0,$2 ); }
		;
...
declaration_type :
		TYPE IDF DEUX_POINTS STRUCT liste_champs FSTRUCT { 
			...
			insererDeclaration( $2,TYPE_S,0,description,taille);
		}
...
un_champ :
		IDF DEUX_POINTS nom_type	{ buffer_ajoute3($1,$3,counter++); }
	 	;
...
declaration_variable :
		VARIABLE suite_declaration_variable
		;
...
variable_suite :
		/* Vide */															{ $$ = NULL; }
		| CROCHET_OUVRANT liste_ind CROCHET_FERMANT variable_suite			{ $$ = ajouterFrere( ajouterFils(creerNoeud( T_TABLE_ELEM ),$2),$4 ); }
		| liste_champs_var variable_suite									{ $$ = ajouterFrere($1,$2); }
		;

liste_ind :
		expression_arith 						{ $$ = ajouterFils( creerNoeud( T_INDICE ),$1 ); }
	  	| expression_arith VIRGULE liste_ind	{ $$ = ajouterFrere( ajouterFils(creerNoeud( T_INDICE ),$1),$3 ); }//{ $$ = ajouterFrere( creerNoeud_i( T_INDICE,$1 ),$3 ); }
	  	;

liste_champs_var :
		POINT IDF							{ $$ = creerNoeud_i( T_STRUCT_ELEM,$2); }
